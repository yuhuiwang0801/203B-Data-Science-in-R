...
sid_dicd <- tbl(con_bq, "d_icd_procedures") 
sid_proc <- sid_proc %>% left_join(sid_dicd, by = c("icd_code", "icd_version")) 
sid_diag <- tbl(con_bq, "diagnoses_icd") 
sid_didc <- tbl(con_bq, "d_icd_diagnoses") 
sid_diag <- sid_diag %>% left_join(sid_didc, by = c("icd_code", "icd_version"))
  ...
  tabPanel("Patient's ADT and ICU stay information", fluid = TRUE,
           sidebarLayout(
             sidebarPanel(
               ...
               selectizeInput("patientID", "Patient ID", choices = patient_id, multiple = FALSE)
             ),
             mainPanel(plotOutput("ADTPlot"))
           )
  )
  )
)
...
demographics <- mimic_icu_cohort %>%
  select(first_careunit, last_careunit, los, admission_type, admission_location, discharge_location, insurance, language, marital_status, race, dod, gender, hospital_expire_flag)
...
server <- function(input, output, session) {
  ...
  reactiveData <- reactive({
    data <- switch(input$variable,
                   "first_careunit" = mimic_icu_cohort %>% select(first_careunit) %>% na.omit() %>% collect(),
                   "last_careunit" = mimic_icu_cohort %>% select(last_careunit) %>% na.omit() %>% collect(),
                   "admission_type" = mimic_icu_cohort %>% select(admission_type) %>% na.omit() %>% collect(),
                   "admission_location" = mimic_icu_cohort %>% select(admission_location) %>% na.omit() %>% collect(),
                   "discharge_location" = mimic_icu_cohort %>% select(discharge_location) %>% na.omit() %>% collect(),
                   "insurance" = mimic_icu_cohort %>% select(insurance) %>% na.omit() %>% collect(),
                   "language" = mimic_icu_cohort %>% select(language) %>% na.omit() %>% collect(),
                   "marital_status" = mimic_icu_cohort %>% select(marital_status) %>% na.omit() %>% collect(),
                   "race" = mimic_icu_cohort %>% select(race) %>% na.omit() %>% collect(),
                   "hospital_expire_flag" = mimic_icu_cohort %>% select(hospital_expire_flag) %>% na.omit() %>% collect(),
                   "gender" = mimic_icu_cohort %>% select(gender) %>% na.omit() %>% collect(),
    ...
    if (input$remove) {
      if ("value" %in% names(data) && is.numeric(data$value)) {
        ...
        data <- data %>%
          ...
          group_by(variable) %>%
          filter({
            bounds <- quantile(value, probs = c(0.25, 0.75), na.rm = TRUE)
            iqr_value <- IQR(value, na.rm = TRUE)
            value > (bounds[1] - 1.5 * iqr_value) & value < (bounds[2] + 1.5 * iqr_value)
          }) %>%
          ungroup()  
      }
    }
...
reactiveData2 <- reactive({
  ...
  data_list <- list(
    sid = sid,
    sid_info = sid_info %>% filter(subject_id == sid) %>% collect(),
    sid_adt = sid_adt %>% filter(subject_id == sid) %>% collect(),
    sid_adm = sid_adm %>% filter(subject_id == sid) %>% collect(),
    ...
    ggplot() +
      ...
      labs(
        title = str_c(
          "Patient ", sid, ", ",
          sid_info$gender, ", ",
          sid_info$anchor_age + year(sid_adm$admittime[1]) - sid_info$anchor_year, " years old, ",

...
write_dataset(icustays_tble, format = "parquet", path = temp_path)
icustays_parquet <- arrow::open_dataset(temp_path)
icustays_parquet
```
```{r}
chartevents_for_icu_stays <- icustays_parquet %>% 
  inner_join(chartevents_filtered, c("subject_id", "stay_id")) %>%
  filter(charttime > intime & charttime < outtime) %>%
  select(subject_id, stay_id, itemid, valuenum) 
chartevents_for_icu_stays
```
```{r}
chartevents_for_icu_stays_collected <- chartevents_for_icu_stays %>% 
  collect()

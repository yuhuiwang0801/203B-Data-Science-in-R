...
mimic_icu_cohort %>%
  ...
  geom_histogram() +
  labs(title = "Length of ICU stay vs Race", x = "Length of ICU stay", y = "Age at intime")
```

```{r}
mimic_icu_cohort %>%
  ggplot(aes(x = los, y = item_50912)) +
  geom_point() +
  labs(title = "Length of ICU stay vs Last available lab measurements before ICU stay", x = "Length of ICU stay", y = "Last available lab measurements before ICU stay")
```

```{r}
ggplot(los, aes(x = los, y = measurement_value)) +
  ...
  facet_grid(rows = vars(measurement_type)) +  # Create a separate plot for each measurement type
  labs(title = "Length of ICU Stay vs. First Vital Measurements",
       x = "Length of Stay (days)", y = "Measurement Value") +
  theme_minimal() +

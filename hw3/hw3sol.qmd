...
ggplot(data = patients_tble, mapping = aes(x = gender, y = anchor_age)) + 
  geom_violin()
```
From graphs, we can see that the median of anchor_age of Female is less than male. The range of anchor_age for both female and male are approximately same.
...
labevents_for_icu_stays <- icustays_tble %>% 
  left_join(labevents_filtered, by = "subject_id") %>%

...
combined_plot <- ggplot() +
  ...
  geom_point(data = sid_procedure, aes(x = chartdate, y = "Procedure", shape = as.factor(long_title.x))) +
  labs(
    x = "Calendar Time",
    y = "",
...
labevents_filtered
```
Change the value to numeric and select the columns we need.
```{r}
labevents_filtered <- labevents_filtered %>%
  mutate(values = as.numeric(replace(value, value == "___", NA))) %>%
  select(subject_id, itemid, storetime, values)
labevents_filtered
```
...
labevents_for_icu_stays <- icustays_tble %>% 
  ...
  select(subject_id, stay_id, itemid, values) 
...
chartevents_for_icu_stays <- chartevents_filtered %>% 
...
```{r}
chartevents_for_icu_stays %>% collect()
```
```{r}

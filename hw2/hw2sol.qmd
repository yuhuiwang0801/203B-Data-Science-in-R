...
```
It takes more than 5 minutes on my computer (elapsed time: 1886.073 seconds), and I abort the program. 
...
```
It partly solved the issue. The run time is much faster than the previous one, but it still takes long time to ingest (elapsed time: 365.601 seconds).
...
```
There are 24855909 lines in the filtered file. The elapsed time is 5.283 seconds.
...
labevents_duckdb_filtered %>% 
    ...
    head(10)
```
The process took elapsed time of 4.034 seconds. The number of rows and the first 10 rows are the same as Q2.3.
